...
// CODE HERE
const changeHelloColor = () => {
  sayHelloBtn.style.backgroundColor = 'black';
  sayHelloBtn.style.color = 'white';
};

sayHelloBtn.addEventListener('mouseover', changeHelloColor);
...
// CODE HERE
const changeHelloColorBack = () => {
  sayHelloBtn.style.backgroundColor = '#EFEFEF';
  sayHelloBtn.style.color = 'black';
  sayHelloBtn.style.border = '1px solid grey';
};

sayHelloBtn.addEventListener('mouseout', changeHelloColorBack);
  ...
  axios
    ...
    .then((response) => {
      ...
      for (let i = 0; i < array.length; i++) {
        ...
        newPara.textContent = String(arr[i]);
        // I understand to be:
        // let newPara = document.createElement('p');
        //   <p></p>
        // <body>
...
const repeatMyParam = (string) => {
  //YOUR CODE HERE
  const repeatText = document.querySelector('#repeat-text');
  axios
    .get(`http://localhost:3000/repeat/${string}`)
    .then((res) => {
...
const getRequest = () => {
  axios
    .get(`http://localhost:3000/query-test?foods=vegetables&mammals=dogs`)
    .then((res) => {
      console.log(res.data);
    })
    .catch((err) => {
...
// PROBLEM 10 ANSWER
// 1. returns 'You sent an empty query!'
// 2. returns 'You sent more than 1 query!' with queries object showing all key-value pairs.
  ...
  axios
    ...
    .then((response) => {
      ...
      for (let i = 0; i < response.data.length; i++) {
        list.appendChild(response.data[i]);
      }
      newPara.textContent = response.data;
    })
    .catch((err) => {
      console.log(err);
    });
...
let foodListBtn = document.querySelector('#food-list-button');
